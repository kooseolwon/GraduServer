...
router.post('/write', async function(req,res){
    ...
    if(!comment_content ||!board_index ||!user_index){
        res.status(400).send({
            message:"fail writing comment from client"
        });
    }else{
        var commentQuery = 'INSERT INTO comment_table (board_index,comment_content,user_index) values (?,?,?)';
        var commentResult = await pool.queryParam_Arr(commentQuery, [board_index,comment_content,user_index]);
...
router.get('/show/:board_index',async function(req,res){
    ...
    if(!board_index){
        ...
        var commentQuery = `SELECT comment_index,comment_content,
        date_format(comment_time ,"%Y-%m-%d %r")AS comment_time,user_name FROM comment_table 
        JOIN user_table ON comment_table.user_index = user_table.user_index WHERE board_index=?`;
